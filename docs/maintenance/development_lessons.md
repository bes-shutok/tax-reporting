...
**See also:** `src/tax_reporting/infrastructure/config.py` (`_REQUIRED_TREATMENT_FLAGS`, `_countries_table_for`, `_enforce_required_treatment_flags`), `src/tax_reporting/domain/jurisdiction.py` (`treatment_*_via_resolver` fields), `docs/maintenance/tax/decision_points/2025.md` DP-019, `tests/unit/application/test_phase_d_flags.py`.
...
**See also:** `tests/unit/application/test_crypto_reporting.py` (`_build_characterization_jurisdiction`, `_ogr_split_jurisdiction`), `tests/end_to_end/test_crypto_derivatives_separation.py` (`_load_with_separation`), `tests/unit/application/test_phase_d_flip_spot_disposal.py` (dedicated flag-on coverage), `docs/maintenance/project-guidelines.md` (decision-point flag defaults).
...
**See also:** `src/tax_reporting/application/crypto_fifo/parsing.py` (`discover_loan_affected_assets` `via_resolver` branch, Invariant 11 clause), `src/tax_reporting/application/crypto/treatment_resolver.py` (`resolve_treatment`, `_normalize_tag`), `tests/unit/application/test_phase_d_flip_loan_repayment.py` (`test_borrow_only_asset_still_in_loan_affected_under_flag`, `test_borrow_only_asset_dropped_under_pure_resolver_delegation`), `docs/history/plans/2026-07-08-th-tx-view-phase-d.md` (Invariant 11).
...
**See also:** `src/tax_reporting/application/token_origin.py` (`_DEFAULT_REWARD_TAGS` / `_DEFAULT_AIRDROP_TAGS` / `_DEFAULT_LP_TAGS` module-level constants and the comment block documenting the parallel-definition state with `treatment_resolver.py`), `src/tax_reporting/application/crypto/treatment_resolver.py` (frozen side; lines 70/73/76 unchanged), `tests/unit/application/test_phase_d_flip_reward_airdrop_lp.py` (`test_inline_literals_extracted_to_constants` scoped to the invariant-safe subset), `docs/history/plans/2026-07-08-th-tx-view-phase-d.md` (Invariant 4 freeze list; Task 7 NOTE line 874 body clause).
