...
- **Reward-sender verification** (`berachain_processor._reward_sub_type`, Attacker F4): rewards from senders NOT in the contract registry classify as `SubType.spam` + review, never a clean `staking`. Exception (2026-08-26): a leg whose `from_address` is the zero address (mint: bridge deposit such as bridged WBTC, or an on-chain mint/airdrop) classifies `SubType.bridge` + review with a bridge-specific reason BEFORE the registry check - the zero address is never registry-listed, so F4's unverified-sender premise does not apply, and CEX->bridge transfer-ins must not read as spam rewards.
