...
Per-lot signal is preserved in `context.review_entries` and the per-lot DEBUG log (file handler at `DEBUG`; aggregate WARNING summaries carry the warning-level signal on the console), and surfaced as Section 5 REVIEW REQUIRED on the Crypto Supplementary sheet; the re-evaluation only re-derives the user-visible aggregated row's flag and reason.
