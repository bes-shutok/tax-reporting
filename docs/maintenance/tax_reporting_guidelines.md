...
- The `Annex` column renders `DerivativesPnLEntry.annex_hint` and the `Código` column renders `operation_code`. Under a PT jurisdiction, `ogr_handler._derivatives_route(country, operator_country)` resolves these per row; under any non-PT jurisdiction both columns render blank.
- Under a PT jurisdiction the sheet warns (logger.warning) when any rendered `Annex` is blank, since a blank route means the residency dispatch failed to resolve.

### Crypto Supplementary Section
- The "1. INCOME CODES REFERENCE" numbered section renders ONLY under `TaxJurisdictionConfig.country == "PT"`. Under any non-PT jurisdiction the ENTIRE numbered section (header, the code/description/Country table, and its surrounding separator) is OMITTED from the sheet, not merely field-blanked. This is a user-visible structural change distinct from leaving individual fields empty.
- The reference table's Country column is sourced from `TaxJurisdictionConfig.country`, not a hardcoded literal. Under PT it lists only the official codes the crypto reward pipeline can actually emit (currently `E25`).
...
**SRG-010**
PT-IRS-specific Modelo 3 fields (crypto reward income codes and derivatives Annex/Código routing) are resolved at the resolution layer and gated on `TaxJurisdictionConfig.country == "PT"`, never emitted unconditionally. Crypto reward income codes: under PT, `interest`/`lending`/`lending interest` resolve to `E25` (Tabela V, Categoria E); staking/reward/airdrop/mining/fork/dividend and unknown types resolve to `""` (no Tabela V code exists for them). Under any non-PT country every crypto reward income code resolves to `""`. Derivatives routing: under PT, resident operator -> Anexo G Quadro 13 (`G51`); non-resident, empty, or `UNKNOWN` operator -> Anexo J Quadro 9.2.B (`G30`); under any non-PT country both `annex_hint` and `operation_code` resolve to `""`. Entity field defaults are the blank route. See PT-C-031 for the counterparty-residency rule and SRG-008 for the taxable-now income placement.
