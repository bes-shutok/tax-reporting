...
- Known EEA-facing CeFi defaults collected so far:
  - `Kraken -> Ireland`
  - `Gate.io -> Malta`
- Numbered crypto-reporting implementation guidance now lives in `docs/maintenance/crypto_reporting_guidelines.md`.

## Wallet and Chain Derivation

- Wallet / platform labels from Koinly are **discovery hints only** for chain and country lookup.
- The final reported `chain` and `country` (País da Fonte) mapping comes from trusted archived sources under `docs/maintenance/tax/crypto-origin/`.
