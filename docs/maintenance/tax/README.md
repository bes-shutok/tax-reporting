...
- Implementation guideline: repository policy or reporting behavior documented under `docs/maintenance/`.
...
- Do not duplicate EU-wide regulation findings inside `laws/pt/crypto-tax/`; keep EU texts in `laws/eu/crypto-tax/` and reference them where needed.
- Do not duplicate Portugal tax-law findings inside `crypto-origin/`.
- Do not duplicate chain/operator domicile mapping files inside `laws/pt/crypto-tax/`.
- If a filing rule depends on an origin mapping, keep the law in `laws/pt/crypto-tax/`, keep the mapping evidence in `crypto-origin/`, and reference both from the domain guideline.
