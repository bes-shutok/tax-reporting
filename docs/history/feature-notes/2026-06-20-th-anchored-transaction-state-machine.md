...
- **Status:**
  ...
  - PHASE C LANDED at `45171a5`; synthetic corpus + one-shot shadow run; 0 treatment-divergence surfaced (recorded in `docs/tmp/phase-c-shadow/RUN_SUMMARY.md`; Phase D receives NO algorithmic treatment-divergence input from this run). The run flagged 540 `requires_review=True` Binance `crypto_deposit` Reward/Airdrop rows (stub-registry coverage gap; Phase D's production registry binding resolves it). Plan archived at `docs/history/plans/completed/2026-07-07-th-tx-view-phase-c.md`. The throwaway shadow script `docs/tmp/phase-c-shadow/shadow_run.py` is gitignored and was deleted at finalization; Phase D restores it via `git show 45171a5:docs/tmp/phase-c-shadow/shadow_run.py > docs/tmp/phase-c-shadow/shadow_run.py` (manual restore - intentional friction per Invariant 3 to prevent silent persistence). Reviews `r1` / `r2` (latest ready: Blocker=0, Medium=0; 3 Low + 3 Monitor items, Phase D-owned).
  - NEXT: Phase D (flip per-treatment classifiers to delegate to the resolver; bind the production stub-registry for Binance `crypto_deposit` Reward/Airdrop rows).
