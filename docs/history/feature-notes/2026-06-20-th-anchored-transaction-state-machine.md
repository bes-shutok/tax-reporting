...
- **Status:**
  ...
  - PHASE C LANDED at `d158904`; synthetic corpus + one-shot shadow run; 0 treatment-divergence surfaced (recorded in `docs/tmp/phase-c-shadow/RUN_SUMMARY.md`; Phase D receives NO algorithmic treatment-divergence input from this run). The run flagged 540 `requires_review=True` Binance `crypto_deposit` Reward/Airdrop rows (stub-registry coverage gap; Phase D's production registry binding resolves it). Plan archived at `docs/history/plans/completed/2026-07-07-th-tx-view-phase-c.md`. The throwaway shadow script `docs/tmp/phase-c-shadow/shadow_run.py` is gitignored and was deleted at finalization; Phase D restored the local leftover for per-flip re-runs and deleted it again at finalization (manual restore - intentional friction per Invariant 3 to prevent silent persistence). Reviews `r1` / `r2` (latest ready: Blocker=0, Medium=0; 3 Low + 3 Monitor items, Phase D-owned).
  - PHASE D LANDED at `0449a9b`; six per-treatment flags default on; legacy adapters bypassed not deleted; Phase E owns deletion. Plan archived at `docs/history/plans/completed/2026-07-08-th-tx-view-phase-d.md`. The six `treatment_*_via_resolver` flags (DP-019) map 1:1 to the six `Treatment` members and default to `true`; flipping one to `false` restores the legacy identification path for that treatment only (rollback granularity). The production WalletKind registry (`application/crypto/wallet_kind_registry.py`) closes the 540-row Binance gap. A required-presence loader guard in `infrastructure/config.py` raises `ConfigurationError` if any of the six flags is absent from a country section of the decision-points TOML. Suite 1745 passed (baseline 1698 + delta 49 incl. parametrized cases). Reviews `r1`-`r9` (latest ready: Blocker=0, Medium=0; 1 Low + 1 Monitor). Phase C SHA reconciliation: `d158904` is authoritative on master/origin; the prior `45171a5` reference was a phantom squash SHA that does not exist on any branch.
  - PHASE E LANDED on branch `2026-07-10-th-tx-view-phase-e` (plan `docs/history/plans/completed/2026-07-10-th-tx-view-phase-e.md`; landing commit `c4de398`). Deleted the six legacy adapters (legacy payment CSV scanner, `_LOAN_PRINCIPAL_TAGS` membership check, inline reward/airdrop/LP tag literals in `token_origin.py`, re-zero snapshot/restore block, and the `derivatives_dedup` legacy internal tag classifier) plus the six `treatment_*_via_resolver` config flags, the `_REQUIRED_TREATMENT_FLAGS` tuple, the `_enforce_required_treatment_flags` loader guard, and the `_countries_table_for` helper. Identification is now resolver-only; there is no legacy rollback path. Byte-identical output verified (Task 9 throwaway script: openpyxl-timestamp-normalized SHA256 over every zip entry + whole-file SHA256 of `shares-leftover.csv` matched the Task 1 Phase-D baseline; the literal whole-file SHA criterion was structurally unsatisfiable because openpyxl stamps fresh `<dcterms:created>`/`<dcterms:modified>` at every `Workbook()` construction - see Task 9 deviation log). Suite 1706 passed. Phase 3 review: 2 consecutive clear rounds (r1 Blocker=0/Medium=0/Low=2, r2 Blocker=0/Medium=0/Low=1; both Low findings fixed in `ecad5bd` and `c0717b4`).
...
| **E. STATUS: LANDED** | Phase E (2026-07-11) deleted the six legacy adapters AND the six `treatment_*_via_resolver` config flags + their required-presence loader guard; identification is resolver-only with no legacy rollback path. Byte-identical output verified against the Phase-D baseline (openpyxl-timestamp-normalized SHA over every zip entry + whole-file SHA of `shares-leftover.csv`). Plan: `docs/history/plans/completed/2026-07-10-th-tx-view-phase-e.md`. | Yes (resolver is the only path) |
