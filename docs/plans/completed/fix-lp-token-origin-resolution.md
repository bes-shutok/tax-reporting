...
- [x] Add three new enum members before `TRANSFER`:
  ```python
  AIRDROP = "airdrop"
...
- [x] Modify `_index_row` `exchange` branch (lines 1025-1030):
  - `"liquidity out"` tag → `LIQUIDITY_WITHDRAWAL`
  - `"liquidity in"` tag → `LIQUIDITY_PROVISION`
  - Others remain `SWAP_CONVERSION`
- [x] Run GREEN: `uv run pytest tests/unit/application/test_crypto_origin_resolver.py -x`
...
- [x] Create `src/shares_reporting/domain/token_origin.py` with:
...
- [x] Update `crypto_reporting.py` to import from `domain.token_origin` and `infrastructure.koinly_parser`
- [x] Update test imports in `test_crypto_origin_resolver.py` and `test_crypto_reporting.py`:
...
- [x] Verify no circular imports: `domain.token_origin` must not import from `application.crypto_reporting`
- [x] Run full suite: `uv run pytest -m unit -x`

## Task 7: Validate against real data
Files:
- `resources/source/koinly2025/` (read-only verification)

- [x] Trace a known LP withdrawal through the resolver:
...
- [x] Verify no regressions on existing origin resolution (exchanges, rewards, transfers)
- [x] Run e2e: `uv run pytest tests/end_to_end/ -x`
