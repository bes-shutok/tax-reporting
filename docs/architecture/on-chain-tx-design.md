...
- One row **per token transfer leg** (native BERA `txlist` leg + every ERC-20 `tokentx` leg + internal native `txlistinternal` legs + ERC-721/1155 `nfttx` legs, the latter decoded only for position-token-registry member contracts - plan `2026-08-24-multi-leg-th-projection`). Multi-leg swaps produce N rows sharing one `tx_hash`.
...
**RESOLVED (2026-08-02):** Migrate the three consumers to key on `(tx_hash, event_id)`, gated by characterization tests on real Koinly data asserting byte-identical output for non-opted-in wallets. Koinly rows set `event_id=None` and behave identically to today; on-chain split rows carry distinct `event_id`. The adapter emits one projected TH row per (Event, leg pair): out and in legs zip by position (`out[0]/in[0]`, `out[1]/in[1]`, ...), unpaired remainder legs emit one-sided rows, and a single-pair Event emits exactly one row byte-identical to the original projection. When one Event projects to multiple rows, rows after the first carry the `.{k}` leg-pair suffix on `event_id` (row 1 keeps the processor's `event_id` verbatim), so no two projected rows share `(tx_hash, event_id)` and a `tx_hash` may legitimately map to multiple rows without the FIFO dedup collapsing them (amended 2026-08-24: previously a single row per Event; now one row per (Event, leg pair), with rows 2+ distinguished by the .{k} event-id suffix). Specifically:
...
1. **B1 + B2 - RESOLVED:** Migrate the three consumers (`crypto_fifo/parsing.py`, `token_origin.py`, `fee_filter.py`) to key on `(tx_hash, event_id)`, gated by characterization tests on real Koinly data. Koinly rows (`event_id=None`) behave identically to today; on-chain split rows carry distinct `event_id`. The adapter emits one projected TH row per (Event, leg pair) with rows after the first carrying the `.{k}` event-id suffix, so no two projected rows share `(tx_hash, event_id)` and a single `tx_hash` may map to multiple rows (amended 2026-08-24: previously a single row per Event; now one row per (Event, leg pair), with rows 2+ distinguished by the .{k} event-id suffix). The two parallel correlation systems (`TxCorrelationKey` + `tx_key`) are both amended with cross-reference comments.
