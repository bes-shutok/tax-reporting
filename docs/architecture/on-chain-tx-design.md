...
- **M1 (fail-loud for opted-in wallets):** adopted - parse/tag/adapter exceptions propagate as `ReportGenerationError`; the broad `except Exception` at `main.py:293` applies ONLY to the collection-only path.
