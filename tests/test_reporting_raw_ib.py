...
from shares_reporting.domain.value_objects import TradeDate, TradeType, get_trade_date, get_currency, get_company
...
def test_comparing_raw_ib():
    ...
    cg_lines = sorted(capital_gains[currency_company], key=lambda x: (x.get_sell_date().year, x.get_sell_date().month, x.get_sell_date().day))
