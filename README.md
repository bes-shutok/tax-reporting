...
- **Popular Crypto Tokens Reference**: `docs/maintenance/tax/popular_crypto_tokens.json` is the popular-token reference file the zero-value-reward detector reads; it additionally carries `stablecoin_pegs` (ticker -> peg fiat) and `payment_tags` reference data consumed by the DP-014 payment-proceeds correction.
