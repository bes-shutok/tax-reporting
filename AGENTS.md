...
Guidance for coding agents. Hard rules first; detail lives in `README.md` and `docs/`.
...
- User-facing output labels use self-explanatory terminology, not terse names inherited from source formats. See `coding_guidelines.md` #6.
...
- `docs/tax/.../official/` keeps only source-origin files; derived notes and numbered guidance belong outside `official/`; `sources.md` records issuing/effective/superseded dates. See `docs/project-guidelines.md` #1.
...
- When AT guidance cites a CIRS paragraph number, verify against the consolidated CIRS PDF — AT documents may predate renumbering amendments. See `docs/project-guidelines.md` #3.
...
- Under `docs/tax/`, use `laws/<jurisdiction>/crypto-tax/` for tax-law archives and `crypto-origin/` for chain/operator domicile archives.
- Adding a boolean flag to `docs/tax/decision_points/<year>.toml` requires the corresponding field on `TaxJurisdictionConfig` in `domain/jurisdiction.py`. See `development_lessons.md` #68.
- Share crypto `País da Fonte` resolution across rewards and capital gains. Never use taxpayer residence.
- Keep `docs/tax/crypto-origin/` source manifest, registry, and decision log synchronized when changing chain/operator mappings.
- Chain derivation uses deterministic normalization and validates against trusted sources in `docs/tax/crypto-origin/`.
...
- Crypto derivatives/futures liquidations reporting losses are disposals of collateral even when liquidating at a loss — correct tax treatment (alienação onerosa), not an error. See `development_lessons.md` #67.
...
- Never write to `docs/review/` (singular); convention is `docs/reviews/` (plural). See `development_lessons.md` #95.
...
- On refactoring branches, fix all in-scope code review findings in the same branch — findings touching changed files or addressing tech debt exposed by the extraction. See `development_lessons.md` #92.
...
- Before changing crypto reporting logic, read `docs/domain/crypto_rules.md`, `docs/domain/crypto_reporting_guidelines.md`, `docs/domain/crypto_implementation_guidelines.md`. Cite PT-C / CRG rule IDs for law-driven changes.
- Before implementing new crypto features, read `docs/domain/crypto_implementation_guidelines.md` for pitfalls.
- Before processing Koinly exports or changing Koinly-related code, read `docs/domain/koinly_guidelines.md` (loan repayment disposal treatment, wrapped-asset repair, Other Gains Report relevance, required settings).
- Before discussing crypto tax treatment, proposing architecture, or advising on Koinly settings, check `docs/tax/decision_points/` first.
- Before changing cross-cutting report-generation behavior, read `docs/domain/tax_reporting_guidelines.md` (also documents Excel report sections) and cite SRG IDs.
- Before writing implementation plans, read `docs/domain/plan_quality_guidelines.md`.
- Before writing/revising repository walkthroughs or presentation artifacts, read `docs/domain/plan_quality_guidelines.md` (presentation-artifact structure and placement).
- When a crypto presentation makes legal/filing claims, verify the current source set in `docs/tax/laws/pt/crypto-tax/sources.md` and cite mirrored official documents.
- Use the authority level and source date in `crypto_rules.md` to check whether a rule may be stale for the current tax year.
- For country-specific tax decision points, see `docs/tax/decision_points/`.
- For private personal tax context supplied by the user, read `docs/personal/facts.md` (gitignored; do not copy into tracked docs unless requested).
...
- **Excel report sections** (Capital Gains, Crypto Gains, Loan Activity, Dividend Income, Report Structure): documented in `docs/domain/tax_reporting_guidelines.md`.
...
- `configparser` INI files: `config.ini` (prod), `tests/config.ini` (test). Four sections: `[COMMON]`, `[EXCHANGE RATES]`, `[SECURITY]`, `[TAX JURISDICTION]` (`TAX_COUNTRY`, `FISCAL_YEAR`, `ZERO_BASIS_REVIEW_THRESHOLD`; defaults PT/2025).
- Update exchange rates annually (e.g., from your national central bank).
- **Law-driven flags** (e.g. `exclude_loan_repayment_gains`) live in `docs/tax/decision_points/<fiscal_year>.toml`, NOT `config.ini`. Both the `.md` and `.toml` sidecar must be updated together. `config.ini` holds only user-preference settings.
- TOML schema: `[meta]` with integer `fiscal_year`, plus `[countries.XX]` boolean-flag tables. Copy `docs/tax/decision_points/2025.toml` for a new year. Missing TOML for the configured `FISCAL_YEAR` raises `MissingDecisionPointsError` (a `ConfigurationError` subclass) at startup; invalid `[TAX JURISDICTION]` raises `ConfigurationError`. Both propagate from `main()` unwrapped so callers distinguish config problems from data problems.

## Testing

- 3-tier: `tests/unit/` (401 unit-marked), `tests/integration/` (10), `tests/end_to_end/` (26 e2e-marked); 451 unmarked (888 total).
- **Do not import pytest fixtures** — injected by name (`tmp_path`, `capsys`, `caplog`, `monkeypatch`, `request`).
...
- **Ruff** is primary linter/formatter (`pyproject.toml`): Python 3.14, line length 120, Google-style docstrings. Rulesets: `E`, `F`, `UP`, `B`, `SIM`, `I`, `N`, `ARG`, `FA`, `DTZ`, `PTH`, `TD`, `FIX`, `RSE`, `S`, `C4`, `PT`, `D`, `PL`.
...
See `docs/project-guidelines.md` #5 for the full missing-vs-invalid rules. Missing data (supplementary info absent): warn with actionable guidance, include record with visible sentinel (`"MISSING_ISIN_REQUIRES_ATTENTION"`, `"UNKNOWN_COUNTRY"`), highlight in Excel, never lose monetary amounts. Internal placeholder sentinels from resolution functions (e.g., `UNKNOWN_OPERATOR_REVIEW_REQUIRED`) must NOT leak to user-facing fields — use the raw input value instead; see `development_lessons.md` #113. Invalid data (format corrupted, non-numeric amounts, missing required columns): fail fast with `FileProcessingError` (row number, field, expected format) using `from e`.
...
Full details, pre-commit checklist, and QA commands: `docs/domain/development_lessons.md`.
