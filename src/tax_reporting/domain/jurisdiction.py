...
class TaxJurisdictionConfig:
    ...
    def derivatives_dedup_enabled(self) -> bool:
        """Whether the per-asset derivatives dedup filter is active.

        Single source of truth for the ``separate_derivatives_reporting AND
        use_other_gains_report`` gate. Callers that need to branch on this
        predicate must read this property rather than re-spelling the
        conjunction, so a future flag addition changes one site.
        """
        return self.separate_derivatives_reporting and self.use_other_gains_report
