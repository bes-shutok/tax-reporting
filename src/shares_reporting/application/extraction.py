...
def parse_data(path: Union[str, Path]) -> TradeCyclePerCompany:
    """
    Legacy function for processing pre-processed CSV files.

    This function maintains backwards compatibility with the existing workflow
    where CSV files have already been pre-processed from raw IB exports.

    Args:
        path: Path to the processed CSV file

    Returns:
        TradeCyclePerCompany with basic Company objects (no ISIN/country data)
    """
    print("Processing pre-processed CSV file (legacy mode)")
    print(path)
