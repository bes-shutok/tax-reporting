...
Provides domain types for resolving token acquisition origins by correlating
capital gains with transaction history. The TokenOriginResolver parses Koinly
transaction history and returns TokenOrigin with acquisition method, source asset,
and platform.
"""
...
class TokenOriginResolver:
    ...
    def _index_withdrawal(self, row: dict[str, str]) -> None:
        ...
        record = _WithdrawalRecord(
            sent_currency=_normalize_asset_ticker(sent_currency),
            sending_wallet=_normalize_platform_name(sending_wallet) if sending_wallet else "",
            tag=tag,
            date_key=date_key,
        )
    ...
    def _index_row(self, row: dict[str, str]) -> None:  # noqa: PLR0911, PLR0912, PLR0915
        ...
        if tx_type == "exchange":
            ...
            normalized_sent_currency = _normalize_asset_ticker(sent_currency) if sent_currency else ""
            normalized_sent_wallet = _normalize_platform_name(sent_wallet) if sent_wallet else ""
